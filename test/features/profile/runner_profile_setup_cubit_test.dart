...
void main() {
  ...
  RunnerProfileSetupCubit buildCubit() => RunnerProfileSetupCubit(_FakeProfileRepository());

  test('Given 새로 생성한 cubit When 초기 상태를 확인하면 Then 기본값과 그에 따른 심박존을 가진다', () {
    final RunnerProfileSetupCubit cubit = buildCubit();

    expect(cubit.state, RunnerProfileSetupState.initial());
    expect(cubit.state.zone, calculator.getHeartRateZone(age: cubit.state.age));
  });

  group('출생 연도 조절', () {
    blocTest<RunnerProfileSetupCubit, RunnerProfileSetupState>(
      'Given cubit When incrementBirthYear 하면 Then 출생 연도가 1 늘고 그에 따른 심박존을 방출한다',
      build: buildCubit,
      act: (cubit) => cubit.incrementBirthYear(),
      expect: () => [RunnerProfileSetupState.initial().copyWith(birthYear: BirthYear(1997))],
    ...
    blocTest<RunnerProfileSetupCubit, RunnerProfileSetupState>(
      ...
      expect: () => [RunnerProfileSetupState.initial().copyWith(birthYear: BirthYear(1995))],
      verify: (cubit) => expect(cubit.state.zone, calculator.getHeartRateZone(age: cubit.state.age)),
    );
    ...
    blocTest<RunnerProfileSetupCubit, RunnerProfileSetupState>(
      ...
      build: buildCubit,
      seed: () => RunnerProfileSetupState.initial().copyWith(birthYear: BirthYear(2012)),
      act: (cubit) => cubit.incrementBirthYear(),
      expect: () => const <RunnerProfileSetupState>[],
    );

    blocTest<RunnerProfileSetupCubit, RunnerProfileSetupState>(
      'Given 최소 출생 연도 상태 When decrementBirthYear 하면 Then 더 감소하지 않고 아무것도 방출하지 않는다',
      build: buildCubit,
      seed: () => RunnerProfileSetupState.initial().copyWith(birthYear: BirthYear(1940)),
      act: (cubit) => cubit.decrementBirthYear(),
      expect: () => const <RunnerProfileSetupState>[],
    );
  });
  ...
  group('프로필 필드 변경 (심박존 영향 없음)', () {
    blocTest<RunnerProfileSetupCubit, RunnerProfileSetupState>(
      ...
      expect: () => [RunnerProfileSetupState.initial().copyWith(gender: Gender.female)],
    );
    ...
    blocTest<RunnerProfileSetupCubit, RunnerProfileSetupState>(
      ...
      expect: () => [RunnerProfileSetupState.initial().copyWith(career: RunningCareer.intermediate)],
    );
    ...
    blocTest<RunnerProfileSetupCubit, RunnerProfileSetupState>(
      ...
      expect: () => [RunnerProfileSetupState.initial().copyWith(weeklyFrequency: WeeklyFrequency(5))],
    );
  });
  ...
  group('완료', () {
    ...
    blocTest<RunnerProfileSetupCubit, RunnerProfileSetupState>(
      ...
      build: () => RunnerProfileSetupCubit(spy),
      act: (cubit) => cubit.complete(),
      expect: () => [
        RunnerProfileSetupState.initial().copyWith(status: RunnerProfileSetupStatus.submitting),
        RunnerProfileSetupState.initial().copyWith(status: RunnerProfileSetupStatus.success),
      ],
      verify: (_) => expect(
        spy.saved,
        RunnerProfile(
    ...
    blocTest<RunnerProfileSetupCubit, RunnerProfileSetupState>(
      ...
      build: () => RunnerProfileSetupCubit(_ThrowingProfileRepository()),
      act: (cubit) => cubit.complete(),
      expect: () => [
        RunnerProfileSetupState.initial().copyWith(status: RunnerProfileSetupStatus.submitting),
        RunnerProfileSetupState.initial().copyWith(status: RunnerProfileSetupStatus.failure),
      ],
    );
    ...
    blocTest<RunnerProfileSetupCubit, RunnerProfileSetupState>(
      ...
      build: buildCubit,
      seed: () => RunnerProfileSetupState.initial().copyWith(status: RunnerProfileSetupStatus.submitting),
      act: (cubit) => cubit.complete(),
      expect: () => const <RunnerProfileSetupState>[],
    );
  });
